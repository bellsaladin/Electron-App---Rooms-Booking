...
var Store = new DevExpress.data.CustomStore({
    ...
    load: function (loadOptions) {
        ...
        $.get(apiBaseURL + '?transform=1'/*, {  
            ...
            var rowsCount = 0;
            if(result.etage) {
                rowsCount = result.etage.length;
            }
            if (loadOptions.requireTotalCount === true)
                deferred.resolve(result.etage, { totalCount: rowsCount });
            else
                deferred.resolve(result.etage)
        });
